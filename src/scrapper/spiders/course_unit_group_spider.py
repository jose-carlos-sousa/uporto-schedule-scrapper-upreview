        ...
        for group_div in group_divs:
            ...
            if group_title:
                ...
                for row in course_rows:
                    ...
                    if link:
                        try:
                            occr_id = link.split("pv_ocorrencia_id=")[1].split("&")[0]

                            sql = """
                            SELECT course_unit.id
                            FROM course_unit_occurrence JOIN course_unit
                            ON course_unit_occurrence.course_unit_id = course_unit.id
                            WHERE course_unit_occurrence.id = ?
                            """
                            self.db.cursor.execute(sql, (occr_id,))
                            result = self.db.cursor.fetchone()
                            course_unit_id = result[0] if result else None
